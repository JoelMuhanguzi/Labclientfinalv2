    ...
    public partial class DefaultLayout : Form
    {
        ...
        public void LoadCurrentFile(string path)
        {
            ...
            XDocument labDoc = XDocument.Load(path);
                ...
                string imagesPath = Path.Combine(labfolder_path, "images");
                string[] files = System.IO.Directory.GetFiles(imagesPath, "*.png");
                System.IO.FileInfo fi = new System.IO.FileInfo(files[0]);              
                lab_Circuit.Image = Image.FromFile(Path.Combine(imagesPath, fi.Name));
                label2.Visible = true;
                ...
                if (ExistingLabList.SelectedIndex > -1)
                {
                    ...
                    else
                    {
                        if (File.Exists(CFGFILE_PATH))
                        {
                            ...
                            DateTime servertime = DateTime.ParseExact(serverdatetime, "MM/dd/yyyy hh:mm tt", null);                        

                            DateTime scheduletime = DateTime.ParseExact(datetime, "MM/dd/yyyy hh:mm tt", null);
            ...
            if(Directory.Exists(CFGFOLDER_PATH))
            {
                if(File.Exists(labReport_path))
                {
                    ...
                    {        
                        ...
                        MessageBox.Show("Lab report saved to Downloads", "Successful Download:", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    }
                    else
                    {
        ...
        {            
            ...
            string dispay_time = timeSpan.ToString(@"hh\:mm\:ss");            
